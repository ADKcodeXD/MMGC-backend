...
export default class MemberService extends BaseService {
  ...
  async resetPwd(params: { email: string; password: string }) {
    if (!params.email || !params.password) return false
    const user = await this.findMemberVoByEmail(params.email)
    if (!user) return false
    const key = crypto.scryptSync(config.AES_PASSWORD || '', config.AES_SALT || '', 16)
    const password = aesEncrypt(params.password, key)
    user.password = password
    delete user.createTime
    await this.memberModel.updateOne({ memberId: user.memberId }, user)
    return user
  }
