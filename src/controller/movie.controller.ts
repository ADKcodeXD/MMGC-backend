...
export default class MovieController {
  ...
  @PutMapping('/sortMovie')
  @Auth([ROLE.ADMIN, ROLE.SUBADMIN, ROLE.COMMITTER, ROLE.GROUPMEMBER], '/sortMovie')
  async sortMovie(@Body() movieParams: Array<{ movieId: number; sortIndex: number }>) {
    const res = await this.movieService.sortMovie(movieParams)
    if (res) return Result.success(null)
    return Result.dataNotFound()
  }
}
